...
- `_isolated_review_analyze.yml` runs a full-SHA-pinned Claude Code Base Action without GitHub/OIDC permission, a proposed-code checkout, or publication tools. The action starts in an empty trusted directory, uses bare and restricted modes with strict MCP configuration, disables project configuration, bundled skills, slash commands, plugins, browser integration, session persistence, and built-in shell, filesystem, web, mutation, and subagent tools, and explicitly allows only the bounded audited `review_context` MCP operations. The action is pinned by immutable commit. Only the validated schema-constrained result and retrieval audit are uploaded; the execution transcript is not published.
